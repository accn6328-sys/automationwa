...
IG_PENDING_FOLLOWUPS_FILE = os.path.join(BASE_DIR, "ig_pending_followups.json")
...
def fetch_page_posts(force=False, limit=20):
...
def fetch_ig_media(force=False, limit=24):
...
def build_ig_dm_body(auto, username=""):
    """Build the FIRST DM body — follow_up_message and follow_up_steps are NOT included here;
    they are sent separately as a second DM when the user replies back."""
    parts = []
    ...
    if dm:
        parts.append(dm)
    # NOTE: link_url is NOT appended here if buttons are enabled — it's sent as a button
    ...
    # NOTE: follow_up_message intentionally excluded — sent as a separate 2nd DM
    return "\n\n".join(p for p in parts if p)


# ── Pending follow-up DM tracking ─────────────────────────────────────────────
# Stores { user_id: {"follow_up": "...", "auto_name": "...", "steps": [...], "step_idx": N} }
# When user replies/taps, the current step is sent and index advances.

def _load_pending_followups() -> dict:
    try:
        if os.path.exists(IG_PENDING_FOLLOWUPS_FILE):
            with open(IG_PENDING_FOLLOWUPS_FILE) as f:
                return json.load(f)
    except Exception:
        pass
    return {}

def _save_pending_followups(data: dict):
    try:
        with open(IG_PENDING_FOLLOWUPS_FILE, "w") as f:
            json.dump(data, f, indent=2)
    except Exception as e:
        print(f"[IG PendingFollowup] Save error: {e}")

def set_pending_followup(user_id: str, follow_up: str, auto_name: str,
                          steps: list = None, step_idx: int = 0):
    """Store a pending follow-up sequence to be served on the user's next reply."""
    data = _load_pending_followups()
    data[user_id] = {
        "follow_up": follow_up,
        "auto_name": auto_name,
        "steps": steps or [],
    ...
    _save_pending_followups(data)
    print(f"  [IG FollowUp] Queued follow-up for user {user_id} (auto: {auto_name}, steps: {len(steps or [])})")

def pop_pending_followup(user_id: str):
    """Return and KEEP the pending follow-up entry (don't delete until steps exhausted)."""
    data = _load_pending_followups()
    return data.get(user_id, None)

def advance_pending_followup(user_id: str):
    """Advance to the next step in the sequence. Remove if no more steps."""
    data = _load_pending_followups()
    entry = data.get(user_id)
    if not entry:
        return
    steps = entry.get("steps", [])
    next_idx = entry.get("step_idx", 0) + 1
    if next_idx >= len(steps):
        data.pop(user_id, None)
    else:
        data[user_id]["step_idx"] = next_idx
    _save_pending_followups(data)

def clear_pending_followup(user_id: str):
    """Remove all pending follow-ups for a user."""
    data = _load_pending_followups()
    data.pop(user_id, None)
    _save_pending_followups(data)
...
def send_ig_dm_with_button(user_id, message, button_label, link_url=""):
    """Send an Instagram DM with a CTA URL button (Text + Button format).
    If link_url is provided, sends a cta_url button; otherwise sends a quick-reply style button.
    Falls back to plain text if the API rejects the button format.
    """
    if not IG_USER_ID:
        return
    if not daily_cap_ok():
    ...
    if link_url:
        # CTA URL button — opens a URL when tapped
        payload = {
            "recipient": {"id": user_id},
            "message": {
                "attachment": {
        ...
        # Quick-reply postback button — fires a postback when tapped
        payload = {
            "recipient": {"id": user_id},
            "message": {
                "text": message,
                "quick_replies": [{
                    "content_type": "text",
                    "title": button_label[:20],
                    "payload": f"FOLLOWUP_TAP:{button_label[:20]}",
                }]
            }
        }
    ...
    result = resp.json()
    if "message_id" in result:
        print(f"  [IG Button DM sent] ✅ button='{button_label}'")
        bump_ig_stat("dms_sent")
        bump_daily_dm()
        sent_ok = True
    else:
        print(f"  [IG Button DM failed] ❌ → {result} — falling back to plain text")
        # Fallback: send as plain text with URL appended
        fallback = message
        if link_url:
            fallback += f"\n\n👉 {button_label}: {link_url}"
        else:
            fallback += f"\n\n👉 {button_label}"
        send_ig_dm(user_id, fallback)
...
def run_ig_automations(trigger_type, text, media_id="", comment_id="", user_id="", username=""):
    ...
    for auto in load_ig_automations():
        ...
        if action in ("dm", "both") and auto.get("dm_message"):
            ...
            # ── Send first DM (with or without button) ──────────────────────
            button_enabled = auto.get("button_enabled") or 0
            buttons = auto.get("buttons") or []
            first_btn = buttons[0] if buttons else None

            if button_enabled and first_btn:
                btn_label = first_btn.get("title", "")[:20]
                btn_url   = first_btn.get("url", "")
                if comment_id:
                    send_ig_private_reply(comment_id, dm_body)  # private reply must be plain text
                    if btn_url:
                        send_ig_dm(user_id, f"👉 {btn_label}: {btn_url}")
                    elif user_id:
                        send_ig_dm_with_button(user_id, "", btn_label, "")
                elif user_id:
                    send_ig_dm_with_button(user_id, dm_body, btn_label, btn_url)
            else:
                if comment_id:
            ...
            # ── Queue follow-up steps sequence ───────────────────────────────
            follow_up_steps = auto.get("follow_up_steps") or []
            # Legacy single follow_up_message support
            legacy_followup = personalize_ig_message(auto.get("follow_up_message", ""), username)

            if follow_up_steps and user_id:
                # Use the steps array (new format)
                set_pending_followup(
                    user_id,
                    follow_up=follow_up_steps[0].get("message", ""),
                    auto_name=auto["name"],
                    steps=follow_up_steps,
                    step_idx=0,
                )
            elif legacy_followup and user_id:
                # Legacy single follow-up
                set_pending_followup(user_id, legacy_followup, auto["name"], steps=[], step_idx=0)
...
def handle_ig_messaging(event):
    ...
    if text:
        ...
        # ── Serve pending follow-up step if this user has one queued ─────────
        pending = pop_pending_followup(sender_id)
        if pending:
            steps = pending.get("steps", [])
            step_idx = pending.get("step_idx", 0)

            if steps and step_idx < len(steps):
                step = steps[step_idx]
                step_msg   = step.get("message", "")
                btn_label  = step.get("button_label", "") or step.get("link_button_label", "")
                step_url   = step.get("link_url", "")

                if step_msg:
                    print(f"  [IG FollowUp] Sending step {step_idx+1}/{len(steps)} to {sender_id} (auto: {pending['auto_name']})")
                    if btn_label:
                        send_ig_dm_with_button(sender_id, step_msg, btn_label, step_url)
                    else:
                        send_ig_dm(sender_id, step_msg)
                    bump_ig_stat("dms_sent")

                # Advance to next step or clear if done
                advance_pending_followup(sender_id)

            elif pending.get("follow_up"):
                # Legacy single follow-up
                print(f"  [IG FollowUp] Sending 2nd DM to {sender_id} (auto: {pending['auto_name']})")
                send_ig_dm(sender_id, pending["follow_up"])
                bump_ig_stat("dms_sent")
                clear_pending_followup(sender_id)
...
<header>
  ...
  <div style="display:flex;align-items:center;gap:16px">
    ...
    <div style="display:flex;align-items:center;gap:6px">
      <span style="font-size:12px;color:#65676b">Count:</span>
      <input type="number" id="bulk-limit" value="20" min="1" max="100" style="width: 55px; padding: 6px; border-radius: 8px; border: 1.5px solid #ccc;">
    </div>
    <button class="btn" style="background:#e6f4ea;color:#137333;border:1px solid #c2e7c9;font-size:13px;padding:8px 14px" onclick="bulkAddLast20()" id="bulk-btn">⚡ Auto-Add Videos</button>
...
// Bulk auto-add videos
async function bulkAddLast20(){
  const limitInput = document.getElementById('bulk-limit');
  const limit = limitInput ? parseInt(limitInput.value) || 20 : 20;
  const btn = document.getElementById('bulk-btn');
  if(!confirm(`This will analyse the latest ${limit} videos/posts from Facebook and Instagram using AI, find each product on radikikk.shop, and create automation rules automatically.\n\nContinue?`))return;
  btn.textContent='⏳ Analysing...';
  btn.disabled=true;
  try{
    const r=await fetch(`/instagram/ui/automations/bulk-last20?limit=${limit}`,{method:'POST'});
    const d=await r.json();
    if(d.ok){
      alert(`✅ Done! Created ${d.created} automation rule(s).\n\n${d.details||''}`);
      location.reload();
    } else {
    ...
    btn.textContent='⚡ Auto-Add Videos';
    btn.disabled=false;
  }
}
</script>
...
<header>
  ...
  <div style="display:flex;align-items:center;gap:16px">
    ...
    <button class="btn" style="background:#fdf2f8;color:#db2777;border:1px solid #fbcfe8;font-size:13px;padding:8px 14px" onclick="bulkAddLast20()" id="bulk-btn">⚡ Auto-Add Videos</button>
    <button class="btn btn-primary" onclick="openModal()">+ Create Automation</button>
  </div>
</header>
...
@app.route("/")
def dashboard():
    return render_template_string(HTML, keywords=load_keywords(), automations=load_automations())
...
@app.route("/instagram/ui/automations/bulk-last20", methods=["POST"])
def ig_bulk_add_last20():
    """
    Fetches latest Instagram and Facebook videos, uses Gemini AI to identify the product
    in each from the caption, searches radikikk.shop for the product URL,
    then creates a DM automation rule for each video with the product link and
    a suitable keyword slug (e.g. 'drawing_robot') appended after 'text=' in the WA order link.
    """
    import re as _re
    ...
    try:
        ...
        if not GEMINI_KEY:
            return jsonify({"ok": False, "error": "GEMINI_API_KEY not set in .env"})
        gemini_client = _genai.Client(api_key=GEMINI_KEY)
    except Exception as e:
        return jsonify({"ok": False, "error": f"Gemini init failed: {e}"})

    SHOPIFY_BASE = "https://www.radikikk.shop"
    WA_NUMBER = os.getenv("WA_NUMBER", "919895138430")

    def _gemini_identify_product(caption: str) -> dict:
        """Use Gemini to extract product name and keyword slug from the post caption."""
        prompt = (
            f"You are an e-commerce assistant for an Indian Shopify store called radikikk.shop.\n"
            f"Given this post caption, identify the product being shown.\n\n"
            f"Caption: {caption[:500]}\n\n"
            "Return ONLY a raw JSON object (no markdown) with exactly these keys:\n"
            "- 'product_name': Short descriptive product name (e.g. '3 in 1 Hair Straightener Comb')\n"
            "- 'shopify_search_query': Best search term to find this product on Shopify (e.g. 'hair straightener comb')\n"
            "- 'keyword_slug': Lowercase underscore slug for WA order keyword (e.g. 'hair_straightener_comb')\n"
        )
        fallback = {"product_name": "Product", "shopify_search_query": caption[:40], "keyword_slug": "product"}
        try:
            for attempt in range(3):
                try:
                    ...
                    if "429" in err or "resource_exhausted" in err:
                        delay_match = _re.search(r'please retry in ([\d\.]+)s', err)
                        sleep_secs = float(delay_match.group(1)) + 1.5 if delay_match else 65
                        print(f"  [Bulk Gemini 429] Sleeping {sleep_secs:.1f}s before retry {attempt+1}...")
                        _time.sleep(sleep_secs)
                    else:
                        raise e
        except Exception as ex:
            print(f"  [Bulk Gemini Error] {ex}")
        return fallback

    def _find_shopify_product_url(search_query: str) -> str:
        """Search radikikk.shop for a product and return its URL."""
        try:
            search_url = f"{SHOPIFY_BASE}/search?q={_urlparse.quote(search_query)}&type=product"
            resp = requests.get(search_url, timeout=10, headers={"User-Agent": "Mozilla/5.0"})
            match = _re.search(r'href="(/products/[a-z0-9_-]+)"', resp.text)
            if match:
                return SHOPIFY_BASE + match.group(1)
        except Exception as e:
            print(f"  [Shopify search error] {e}")
        return SHOPIFY_BASE

    # ==========================================
    # PART A: INSTAGRAM AUTOMATION
    # ==========================================
    media_list = fetch_ig_media(force=True, limit=limit * 2)
    if media_list:
        ig_videos = [m for m in media_list if "video" in m.get("media_type", "").lower() or "reel" in m.get("media_type", "").lower()]
        if not ig_videos:
            ig_videos = media_list
        ig_videos = ig_videos[:limit]

        existing_autos = load_ig_automations()
        existing_post_ids = {pid for a in existing_autos for pid in a.get("post_ids", [])}

        for media in ig_videos:
            media_id  = media.get("id", "")
            caption   = media.get("message") or media.get("caption") or "Product video"
            thumbnail = media.get("thumbnail", "")

            if media_id in existing_post_ids:
                continue

            print(f"  [Bulk IG] Analysing media {media_id}: {caption[:60]}")
            product_info = _gemini_identify_product(caption)
            product_name = product_info.get("product_name", "Product")
            search_query = product_info.get("shopify_search_query", caption[:40])
            keyword_slug = product_info.get("keyword_slug", "product")

            product_url = _find_shopify_product_url(search_query)

            wa_order_link = f"https://wa.me/{WA_NUMBER}?text={_urlparse.quote(keyword_slug)}"
            dm_message = (
            ...
            rule = {
                ...
                "reply":            f"Check your inbox, details sent! 📥",
                "action":           "both",
            ...
            existing_autos.append(rule)
            created_ig += 1
            details_lines.append(f"• [Instagram] {product_name} → {product_url}")
            print(f"  [Bulk IG] ✅ Created rule for '{product_name}'")

        if created_ig > 0:
            save_ig_automations(existing_autos)

    # ==========================================
    # PART B: FACEBOOK AUTOMATION
    # ==========================================
    fb_posts = fetch_page_posts(force=True, limit=limit * 2)
    if fb_posts:
        fb_videos = [p for p in fb_posts if "video" in p.get("media_type", "").lower() or "reel" in p.get("media_type", "").lower()]
        if not fb_videos:
            fb_videos = fb_posts
        fb_videos = fb_videos[:limit]

        existing_fb_autos = load_automations()
        existing_fb_post_ids = {pid for a in existing_fb_autos for pid in a.get("post_ids", [])}

        for media in fb_videos:
            media_id  = media.get("id", "")
            caption   = media.get("message") or "Product video"
            thumbnail = media.get("thumbnail", "")

            if media_id in existing_fb_post_ids:
                continue

            print(f"  [Bulk FB] Analysing post {media_id}: {caption[:60]}")
            product_info = _gemini_identify_product(caption)
            product_name = product_info.get("product_name", "Product")
            search_query = product_info.get("shopify_search_query", caption[:40])
            keyword_slug = product_info.get("keyword_slug", "product")
            ...
            existing_fb_autos.append(rule)
            created_fb += 1
            details_lines.append(f"• [Facebook] {product_name} → {product_url}")
            print(f"  [Bulk FB] ✅ Created rule for '{product_name}'")

        if created_fb > 0:
            save_automations(existing_fb_autos)

    return jsonify({
        "ok":      True,
        "created": created_ig + created_fb,
        "details": "\n".join(details_lines) if details_lines else "All videos already had automations."
    })
